...
const SETTINGS_USAGE_COPY = {
  ...
  'zh-TW': {
    ...
    cacheDetail: (miss, read, creation) => `新 ${miss} / 命中 ${read} / 建立 ${creation}`, viewAria: '使用統計檢視', tabs: ['請求日誌', '供應商統計', '模型統計', '工具統計', '定價設定'],
    filtersAria: '請求記錄篩選', filterPlaceholder: '按模型或工具篩選…', filterAria: '按模型或工具篩選請求記錄', statusAria: '請求狀態篩選',
    statuses: ['全部狀態', '成功', '錯誤'], details: '詳情記錄', detailsAria: '顯示使用統計詳情記錄', recordCount: (count) => `共 ${count} 條記錄`, clearFilters: '清除篩選',
