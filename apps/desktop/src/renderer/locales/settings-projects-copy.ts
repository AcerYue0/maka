...
const SETTINGS_PROJECTS_COPY_BY_LOCALE = {
  ...
  'zh-TW': {
    runtimeHost: {
      ...
      plaintextAcknowledgementHelp: '訪問憑據和資料可能被同一網路中的第三方截獲',
      ...
      startBeforeChangingAccess: '請先啟動 Runtime Host 服務，再修改訪問權限。',
      revokeCredential: '撤銷',
      revokeCredentialConfirm: (name: string) => `撤銷 ${name} 的訪問權限？`,
      revokeCredentialConfirmBody: '使用此憑據的客戶端會立即斷開連線，並可能中斷正在進行的工作。',
      accessActionFailed: '無法管理訪問權限',
      back: '返回',
      remove: '移除',
      empty: '還沒有遠端 Host',
      ...
      pairingRecoveryDescription: '遠端 Host 恢復連線後，可在此繼續完成配對。',
