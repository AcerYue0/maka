...
const zhTwCopy = {
  ...
  readiness: {
    unscaffolded: { label: '未開放', detail: '該平臺目前不可作為遠端串接渠道。', tone: 'neutral' },
  ...
  planned: { label: '未開放', detail: '該平臺目前不會儲存為遠端串接渠道或定時任務投遞目標。', tone: 'neutral' as const },
  status: {
    disabled: '開關關閉', noToken: '等待填寫 Bot Token', missingFeishuCredentials: '等待填寫飛書 App ID 或 App Secret',
    feishuDomainRequired: '飛書憑據有效，等待填寫事件訂閱域名', feishuEventsNotConnected: '飛書憑據有效，等待事件回撥串接',
    unavailable: '該平臺目前不可作為遠端串接渠道', stopped: '監聽已停止', detailsInLogs: '執行態詳情請見日誌',
    polling: '長輪詢', gateway: '事件通道', webhook: 'Webhook', none: '無',
  },
  overview: {
    loadFailed: '遠端串接狀態載入失敗', reload: '重新載入', active: '正在使用', sortHint: '按需要處理、最近活動排序',
    empty: '還沒有正在使用的渠道', emptyHelp: '從下方選擇一個訊息平臺開始設定。', more: '串接更多渠道', choose: '選擇平臺開始設定',
  ...
  detail: {
    unavailableHint: '該平臺未開放，暫不能啟用。', scanFirstHint: '先掃碼串接後才能啟用。', testFirstHint: '先測試並連線後才能啟用。',
    back: '返回遠端串接', configDocs: '檢視設定文件', enableAria: (name: string) => `啟用${name}渠道`, listening: '正在監聽新訊息', healthy: '連線正常，無需處理。',
    actionsAria: (name: string) => `${name}渠道操作`, quickBind: '快捷繫結', scanLogin: '掃碼登入', scanConnect: '掃碼串接', disconnecting: '斷開中…', disconnectWechat: '斷開微信登入', bridgeQr: '本機橋接二維碼',
    ...
    statusRefreshFailed: '執行狀態重新整理失敗', latestFailure: '最近一次失敗', latestFailureDetail: '請檢查設定、網路和執行日誌後重試。', savedButNotConnected: '憑據已儲存，但連線尚未成功啟動。', setupMethod: '串接方式', connectionSettings: '連線設定', localCredentials: '憑據僅儲存在本機', autosave: '自動儲存',
    ...
    beginQuickBind: '開始快捷繫結', scanWith: (name: string) => `使用${name}掃碼串接`, planned: '這個平臺目前只作為平臺清單展示，不會進入可用渠道，也不會儲存為定時任務投遞目標。',
    credentialsSaved: (name: string) => `${name}憑據已儲存`, scanComplete: (name: string) => `${name}已完成掃碼串接`, savedAndConnected: '憑據已安全儲存並開始連線',
  ...
  onboarding: {
    providers: {
      ...
      wecom: { title: '設定企業微信', ariaLabel: '設定企業微信掃碼串接', qrAlt: '設定企業微信二維碼', subtitle: '快捷繫結會自動建立並連線企業微信機器人', waiting: '開啟企業微信，掃描二維碼完成機器人建立', scanned: '已掃碼，請在企業微信中完成確認' },
      ...
      qq: { title: '設定 QQ', ariaLabel: '設定 QQ 掃碼串接', qrAlt: '設定 QQ 二維碼', subtitle: '使用手機 QQ 掃碼建立並繫結機器人', waiting: '請使用手機 QQ 掃描二維碼並確認繫結', scanned: '已掃碼，請在 QQ 中完成確認' },
    },
    ...
    connected: (name: string) => `${name} 已連線`, connectedWarning: '憑據已儲存，但連線尚未成功啟動。', expired: '二維碼已過期，請重新生成', denied: '授權已取消，請重新生成二維碼', cancelled: '掃碼串接已取消', failed: '掃碼串接失敗，請重試', preparing: '準備掃碼串接…',
  },
  wechat: {
