...
const SHELL_COPY_BY_LOCALE = {
  ...
  'zh-TW': {
    ...
    sessionSettingsActions: {
      ...
      modelSwitchedTitle: '已切換目前任務模型',
      modelSwitchedDescription: (from, to) => `${from} → ${to}`,
      modelFailedTitle: '切換模型失敗',
      modelFallback: '模型暫時無法切換，請稍後重試。',
      modelRecoveryHint: '如果所選連線需要登入或 API Key，請到 設定 · 模型 補齊後重試。',
      thinkingUpdatedTitle: '已更新思考級別',
      thinkingDefault: '預設',
      thinkingLabels: {
        off: '關',
        minimal: '最少',
    ...
    errorBoundary: {
      ...
      copyReport: '複製診斷資訊',
      title: 'Maka 渲染層崩潰了',
      descriptionBeforeRetry: '已捕獲一次未處理的 React 異常。下面是錯誤摘要；點',
      retry: '重試',
      descriptionBeforeReload: '清掉這次崩潰，',
      reload: '重新載入',
      descriptionAfterReload: '會重新整理整個視窗。需要交接時先複製診斷資訊。',
      errorDetails: '錯誤詳情',
      componentStack: '元件棧',
      clipboardFailure: '剪貼簿不可用或被系統拒絕；可以手動選擇上面的錯誤摘要。',
    },
    commandPalette: {
      label: '命令面板',
      searchLabel: '命令面板搜尋',
    ...
    app: {
      ...
      permissionModeChanging: '權限模式正在切換，完成後再繼續操作。',
