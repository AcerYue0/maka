...
const SETTINGS_PREFERENCES_COPY_BY_LOCALE = {
  'zh-CN': {
    ...
    general: {
      incognito: '隐身模式', incognitoHelp: '开启后暂停本地记忆读写、联网搜索和定时任务触发。', enableIncognito: '启用隐身模式', incognitoFailed: '隐身模式切换失败', notifications: '完成时发送系统通知', notificationsHelp: '窗口不在前台时，在回答完成或出错后发送桌面通知。', notificationsFailed: '通知设置切换失败', workspaceInstructions: '遵循项目指令', workspaceInstructionsHelp: '自动读取每个项目中已有的 AGENTS.md、CLAUDE.md 或 GEMINI.md；文件仍由各自项目管理。', workspaceInstructionsFailed: '项目指令设置切换失败', workHub: '启用 WorkHub', workHubHelp: '在一个入口查看已有工作，并将新输入保守地送往普通任务。', workHubFailed: 'WorkHub 设置切换失败', updateFailed: '设置未生效，请稍后重试。',
  ...
  'zh-TW': {
    ...
    appearance: {
      ...
      appIconLabels: { default: '經典', mono: '單色', 'sky': '原色天藍', 'cyan': '青藍', 'ice': '冰藍漸變', 'pale-inverted': '淡底深標', 'ink': '墨黑', 'paper': '紙白', 'graphite': '石墨', 'pencil-kraft': '鉛筆・牛皮紙', 'pencil-sky': '鉛筆・天藍', 'pencil-navy': '鉛筆・深藍', 'alpine': '晴空雪山', 'dusk': '黃昏', 'night': '夜山', 'forest': '蒼綠' },
      appIconHelp: { default: 'Maka 預設品牌圖示', mono: '灰階版本，Dock 裡更安靜', 'sky': '幾何 M 標，品牌藍', 'cyan': '偏青的藍', 'ice': '由淺到深的藍色漸變', 'pale-inverted': '淡藍底配深藍標', 'ink': '黑底白標，對比最強', 'paper': '白底黑標', 'graphite': '白底黑標，筆尖為灰', 'pencil-kraft': '鉛筆意象，牛皮紙底', 'pencil-sky': '鉛筆意象，天藍底', 'pencil-navy': '鉛筆意象，深藍底', 'alpine': '雪頂山峰，晴空底', 'dusk': '雪頂山峰，黃昏底', 'night': '雪頂山峰，夜色底', 'forest': '雪頂山峰，蒼綠底' },
      appIconGroups: {
    ...
    about: {
      ...
      updateHelp: '後臺也會定期檢查；需要重啟安裝時側欄會提示。',
      updateDevBuildHelp: '本地開發版不檢查 GitHub 釋出更新。請使用正式安裝包。',
      updateIdle: '尚未檢查更新。',
      updateNotAvailable: '已是最新版本。',
