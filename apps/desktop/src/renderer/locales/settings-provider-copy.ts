...
const zhTwCapabilitiesCopy = {
  ...
  thinkingBulkHelp: '勾選寫入下方全部已啟用模型，取消勾選則從全部模型移除；其餘檔位不受影響。',
  thinkingBulkTrigger: '應用到全部模型',
  thinkingBulkCoverage: (declared: number, total: number) =>
...
const zhTwCopy = {
  detail: {
    ...
    modelKey: '模型金鑰', pasteModelKey: '貼上模型金鑰', getModelKey: '獲取模型金鑰', saving: '儲存中…',
    ...
    credentialsHelpAccount: '登入令牌只儲存在本機。',
    ...
    copilotImportFailed: '匯入 GitHub Copilot 登入失敗', copilotLoggedIn: 'GitHub Copilot 已登入', copilotWaiting: '等待相容 GitHub 憑據',
    copilotLoggedInDetail: '若帳號或組織策略變化，可重新匯入相容憑據。', copilotWaitingDetail: '設定具有 Copilot Requests 權限的憑據後從本機安全匯入。',
    reimport: '重新匯入', importCredential: '匯入相容憑據', login: '登入', loggingIn: '登入中…', relogin: '重新登入',
    oauthReloginDetail: '若請求提示需要重新登入，點這裡重新走一遍授權。',
    oauthStartDetail: '點下方按鈕開啟瀏覽器完成登入，授權成功後會自動重新整理這裡的狀態。',
    enabledModels: '啟用的模型',
    searchModels: '搜尋模型', selectAllModels: '全部啟用',
  ...
  shared: {
    ...
    lastTest: {
      ...
      '当前账号无可用 codex 模型。': '目前帳號無可用 Codex 模型。', 'codex 模型列表获取失败。': 'Codex 模型列表獲取失敗。',
  ...
  panel: {
    tabs: { all: '全部', recommended: '推薦', accounts: '帳號', plans: '模型計劃', api: 'API', aggregators: '聚合服務', local: '本地' },
    loadFailed: '載入模型連線失敗', loadingAria: '正在載入模型供應商', connections: '模型連線',
    retry: '點選重試。', empty: '還沒有模型連線',
    emptyHelp: '從下方選擇一種連線方式開始。', default: '預設', setDefault: '設為預設', setDefaultTitle: '讓新任務預設使用這個連線', setDefaultPending: '設定中…', setDefaultFailed: '設為預設失敗', addHelp: '選擇帳號登入、模型計劃、API、聚合服務或本地執行時。',
    categoriesAria: '模型供應商分類', searchPlaceholder: '搜尋服務商', searchAria: '搜尋模型服務商', noMatch: '沒有匹配的服務商', clearSearch: '清除搜尋',
    createSubtitle: '完成必要設定後，連線會出現在模型頁上方。', connection: '模型連線',
    count: (value: number) => `· ${value}`, connectTitle: (name: string) => `連線 ${name}`,
    chipAria: (name: string, provider: string, isDefault: boolean, status?: string) => `模型連線：${name}，供應商：${provider}${isDefault ? '，預設連線' : ''}${status ? `，${status}` : ''}`,
    addConnection: '新增連線', category: '分類', backToList: '返回模型連線', backToCatalog: '返回服務商列表',
  },
  catalog: {
    unavailable: '未開放',
  ...
  oauthFlow: {
    ...
    reverifyFailedRetry: '重新驗證失敗，請稍後重試。', serviceUnavailable: '登入服務暫時不可用，請檢查網路後重試。',
  ...
  oauthSection: {
    ...
    openingBrowser: '開啟瀏覽器…', logout: '退出登入', loggingOut: '退出中…',
    ...
    reimport: '重新匯入', importCredential: '匯入相容憑據', verifying: '驗證中…', reverify: '重新驗證', removing: '移除中…', removeLocal: '移除本地登入',
    loadingAccount: '正在載入帳號狀態…', authorizing: '請在彈出的瀏覽器視窗完成登入。', refreshing: '正在重新整理訪問令牌…', refreshTokenFailed: '令牌重新整理失敗，請重新登入。',
    cardAria: (name: string, status: string | undefined, description: string) => `開啟 OAuth 登入：${name}${status ? `，狀態：${status}` : ''}，${description.replace(/[。.!！？?]+$/u, '')}`,
    connectTitle: (name: string) => `連線 ${name}`, login: (name: string) => `登入 ${name}`, signedOut: (name: string) => `${name} 尚未登入。`,
