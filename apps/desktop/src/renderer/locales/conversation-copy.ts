...
const COPY = {
  ...
  'zh-TW': {
    ...
    turnError: { unknown: '未知錯誤', contextOverflow: '上下文視窗已超出限制', timeout: '請求超時', auth: '鑑權失敗', providerBilling: '模型服務計費受限', providerCapacity: '模型服務暫時滿載，請稍後重試或切換模型', rateLimit: '觸發模型速率限制', network: '網路錯誤', provider: '模型服務返回錯誤', stepCap: '達到工具步驟上限', tool: '工具呼叫失敗', permission: '等待權限確認', restarted: '本地應用重啟，上一輪沒有完成', sandboxBoundaryClosed: '本地應用重啟，等待確認的「允許訪問工作區以外的內容」請求已按拒絕關閉', recovery: { safeResume: '檢查目前狀態後，可嘗試安全恢復', stepCap: '任務可能尚未完成，可以繼續', toolError: '先檢查工具結果，再決定是否重試', connection: '先檢查模型連線或登入狀態', partial: '已保留部分輸出，可從這裡繼續', toolRecord: '工具記錄已保留，重試前先看結果', retry: '沒有執行工具，可直接重試', capacity: '模型服務暫時滿載，請等待幾分鐘或切換模型後重試', contextOverflow: '上下文仍超出限制，請減少附件或開啟新任務', sandboxBoundaryClosed: '訪問範圍沒有放開，重試本輪後可重新決定' } },
  },
  en: {
